...
async def top_up_wallet(callback_query: types.CallbackQuery):
    amount_map = {
        "top_up_wallet_100": 100,
        "top_up_wallet_500": 500,
        "top_up_wallet_1000": 1000
    }
    amount = amount_map.get(callback_query.data, None)
    if amount:
        await send_invoice(bot, callback_query.message.chat.id, amount, "wallet_funding_payload")
    else:
        await callback_query.message.answer("Неверная сумма для пополнения.")
